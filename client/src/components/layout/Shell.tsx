...
const Sidebar = ({ className, collapsed, onToggle }: { className?: string, collapsed?: boolean, onToggle?: () => void }) => {
  ...
  return (
    <div className={cn("flex flex-col h-full bg-sidebar border-r border-sidebar-border transition-all duration-300", className)}>
      <div className={cn("p-6", collapsed && "p-4")}>
        <div className={cn("flex items-center mb-8", collapsed ? "justify-center flex-col gap-4" : "justify-between")}>
          ...
          <Button
            ...
            size="icon"
            onClick={onToggle}
            className={cn("text-muted-foreground hover:text-foreground h-6 w-6", collapsed && "h-8 w-8")}
          >
      ...
      <div className="border-t border-sidebar-border bg-black/20">
        <div className={cn("p-6", collapsed && "p-4")}>
