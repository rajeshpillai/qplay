...
import { Play, RotateCcw, CheckCircle2, AlertTriangle, Terminal, Code2, FileCode, ArrowLeft, BookOpen, ExternalLink } from "lucide-react";
                   ...
                   {status === "success" && (
                     <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-md animate-in zoom-in duration-300">
                       <div className="flex items-center gap-2 text-green-400 font-bold mb-1">
                         <CheckCircle2 className="h-5 w-5" /> Mission Accomplished
                       </div>
                       ...
                       <Button 
                         ...
                         onClick={() => setLocation("/modules/k6")}
                       >
                         Return to Lab List
                       </Button>
                     </div>
                 ...
                 <div className="p-4 space-y-4">
                   ...
                   <div className="space-y-4">
                     <div className="space-y-2">
                       <h4 className="text-xs font-bold text-primary uppercase">Checks</h4>
                       <div className="bg-black/40 p-2 rounded border border-white/10 font-mono text-[10px] text-muted-foreground">
                         <span className="text-blue-400">check</span>(res, &#123;<br/>
                         &nbsp;&nbsp;<span className="text-green-400">'status is 200'</span>: (r) =&gt; r.status === 200<br/>
                         &#125;);
                       </div>
                     </div>

                     <div className="space-y-2">
                       <h4 className="text-xs font-bold text-primary uppercase">Thresholds</h4>
                       <div className="bg-black/40 p-2 rounded border border-white/10 font-mono text-[10px] text-muted-foreground">
                         <span className="text-purple-400">thresholds</span>: &#123;<br/>
                         &nbsp;&nbsp;<span className="text-green-400">'http_req_duration'</span>: [<span className="text-yellow-400">'p(95)&lt;500'</span>],<br/>
                         &nbsp;&nbsp;<span className="text-green-400">'http_req_failed'</span>: [<span className="text-yellow-400">'rate&lt;0.01'</span>],<br/>
                         &#125;
                       </div>
                     </div>

                     <div className="space-y-2">
                       <h4 className="text-xs font-bold text-primary uppercase">Sleep</h4>
                       <div className="bg-black/40 p-2 rounded border border-white/10 font-mono text-[10px] text-muted-foreground">
                         <span className="text-purple-400">import</span> &#123; sleep &#125; <span className="text-purple-400">from</span> <span className="text-green-400">'k6'</span>;<br/>
                         <br/>
                         <span className="text-blue-400">sleep</span>(1); <span className="text-gray-500">// Wait 1s</span>
                       </div>
                     </div>
                   </div>
                   
                   <Button variant="outline" className="w-full text-xs" onClick={() => window.open('https://k6.io/docs', '_blank')}>
