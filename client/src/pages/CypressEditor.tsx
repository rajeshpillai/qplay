...
import { Play, RotateCcw, CheckCircle2, Terminal, Code2, FileCode, ArrowLeft, BookOpen, ExternalLink, TestTube2 } from "lucide-react";
...
export default function CypressEditor() {
  ...
  const labId = params?.labId || "selectors";
  const lab = CYPRESS_LABS.find(l => l.id === labId) || CYPRESS_LABS[0];
  ...
  const runTest = () => {
    setIsRunning(true);
    setStatus("running");
    setOutput(["Starting Cypress...", "Discovering specs...", `Running: ${lab.id}.spec.js`]);
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-muted-foreground text-sm">Cypress Training</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => setLocation("/modules/cypress")}>
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="w-full justify-start rounded-none bg-transparent p-0 h-auto">
                  ...
                  <TabsTrigger
                    ...
                    className="data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none px-4 py-3"
                  >
                    Test Runner {output.length > 0 && `(${output.length})`}
                  </TabsTrigger>
                  <TabsTrigger
                    value="docs"
                    className="data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none px-4 py-3"
                  ...
                  {status === "success" && (
                    <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-md animate-in zoom-in duration-300">
                      <div className="flex items-center gap-2 text-green-400 font-bold mb-1">
                        <CheckCircle2 className="h-5 w-5" /> All Specs Passed
                      </div>
                      <p className="text-xs text-green-400/80">
                        Great job! Your tests are now resilient to refactors.
                      </p>
                      <Button
                        variant="outline"
                        size="sm"
                        className="mt-3 w-full border-green-500/30 hover:bg-green-500/20 text-green-400"
                        onClick={() => setLocation("/modules/cypress")}
                      >
                        Return to Lab List
                      </Button>
                    </div>
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <h4 className="text-xs font-bold text-primary uppercase">Selectors</h4>
                      <div className="bg-black/40 p-2 rounded border border-white/10 font-mono text-[10px] text-muted-foreground">
                        <span className="text-gray-500">// Get by data-testid</span><br />
                        <span className="text-blue-400">cy.get</span>(<span className="text-green-400">'[data-testid="submit"]'</span>);
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-black/40 p-2 rounded border border-white/10 font-mono text-[10px] text-muted-foreground">
                        <span className="text-blue-400">cy.get</span>(<span className="text-green-400">'.alert'</span>)<br />
                        &nbsp;&nbsp;.<span className="text-yellow-400">should</span>(<span className="text-green-400">'be.visible'</span>)<br />
                        &nbsp;&nbsp;.<span className="text-yellow-400">and</span>(<span className="text-green-400">'contain'</span>, <span className="text-green-400">'Success'</span>);
                      </div>
                    </div>
                  </div>

                  <Button variant="outline" className="w-full text-xs" onClick={() => window.open('https://docs.cypress.io', '_blank')}>
                    <ExternalLink className="mr-2 h-3 w-3" /> Full Documentation
                  </Button>
                </div>
              )}
            </div>
